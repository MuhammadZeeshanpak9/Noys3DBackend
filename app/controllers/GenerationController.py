...
async def _submit_tripo_task(api_key: str, prompt: str = "", file_token: str = None, file_ext: str = "jpeg") -> Optional[str]:
    ...
    # Style suffix appended to all text prompts for consistent grey clay output
    STYLE_SUFFIX = ", neutral grey clay material, no color, no texture, uniform grey surface, plain white background, clean studio render"
...
async def generate_model(request: Request, background_tasks: BackgroundTasks):
    try:
        ...
        if settings.tripo_api_key:
            ...
            # Store task_id so get_generation can check Tripo on-demand per frontend poll
            _active_tasks[generation_id] = task_id

            # Also poll in background as a redundant fallback
...
async def get_generation(request: Request, generation_id: str):
    try:
        ...
        # On-demand Tripo check: if still processing and we have the task_id, ask Tripo now.
        # This makes each frontend poll actively drive progress rather than waiting for the
        # background task, which Railway can kill during long generations.
